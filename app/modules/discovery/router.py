...
def start_discovery_session(
    ...
    response.set_cookie(
        ...
        httponly=True,
        samesite="lax",
        secure=settings.session_secure_cookie or (settings.app_env in ("production", "staging")),
        max_age=30 * 86400,
    )
    # Expose token in response headers ONLY for automated testing / development clients
